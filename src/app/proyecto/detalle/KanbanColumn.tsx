...
function KanbanColumn({ estado, tareas, proyectoId }) {
  ...
  return (
    <div className={`flex flex-col p-4 border rounded ${getBorderColor(estado)}`}>
      ...
      {tareasEnColumna.map((tarea) => (
         <Link
         key={tarea.id}
         href={`/proyecto/tarea/detalle?proyectoId=${proyectoId}&tareaId=${tarea.id}`}
       >
           <div className="mb-4 block max-w-sm p-6 bg-white border border-gray-50 rounded-lg shadow hover:bg-gold-100 dark:bg-yellow-100 dark:border-gray-700 dark:hover:bg-yellow-200">
             <p>#{tarea.id}</p>
